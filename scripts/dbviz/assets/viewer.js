...
(function () {
  ...
  function renderGrid(columns, rows) {
    ...
    var tabulatorColumns = columns.map(function (c) {
      // El nombre de columna viene crudo de la BD y Tabulator inserta
      // `title` como HTML (innerHTML) en el header del grid, no como texto:
      // debe escaparse igual que cualquier otro valor proveniente de datos
      // de usuario (ver regla de seguridad en la cabecera de este archivo).
      return {
  ...
  function renderStats() {
    ...
    var palette = ["#2f6fed", "#33b679", "#f0a63a", "#e5534b", "#9b59d0", "#1abcbf", "#e07ba0", "#8a8f99"];

    STATE.charts.push(
      new Chart(document.getElementById("dbviz-chart-rows"), {
        type: "bar",
        data: {
          labels: labels,
          datasets: [
    ...
    STATE.charts.push(
      new Chart(document.getElementById("dbviz-chart-cols"), {
        type: "bar",
        data: {
          labels: labels,
          datasets: [
    ...
    STATE.charts.push(
      new Chart(document.getElementById("dbviz-chart-types"), {
        type: "pie",
        data: {
          labels: typeLabels,
          datasets: [
  ...
  function renderAbout() {
    ...
    var warnings = m.warnings || [];
    if (warnings.length > 0) {
      html += "<h3>Avisos (" + warnings.length + ")</h3><ul>";
