...
def extract_sqlite(source: dict, cfg: dict, tmpdir: str, forced_engine: str = "auto") -> dict:
    ...
    try:
        copy_path, had_wal = _copy_source_files(src_path, tmpdir)
    ...
    try:
        con, used_immutable = _open_readonly(copy_path)
    ...
    try:
        try:
            ...
            # La corrupción de un archivo SQLite a menudo solo se detecta al
            # leer más allá de la cabecera (el `SELECT 1` de _open_readonly
            # no la detecta) - se marca E-CORRUPT aquí (research §8.5 / SPEC §8).
            source["error"] = "BD corrupta o ilegible"
            source["warnings"].append(util.W(E_CORRUPT, str(e)))
            source["hasData"] = False
            source["tables"] = []
...
def _extract_sqlite_via_cli(source: dict, copy_path: str, cfg: dict, wal_merged: bool) -> dict:
    try:
        tables_meta = _sqlite3_cli_query_json(
            copy_path,
            "SELECT name, type, sql FROM sqlite_master "
    ...
    for tmeta in tables_meta:
        ...
        try:
            ...
            total = _sqlite3_cli_query_json(
                copy_path, f"SELECT COUNT(*) AS n FROM {qname};")[0]["n"]
